...
def _get_moves(lines) -> list[list[list[int, int]]]:
    ...
    return [
        ...
        if l[0][0] == l[1][0]
        or l[0][1] == l[1][1]
        or abs(l[0][0] - l[1][0]) == abs(l[0][1] - l[1][1])
    ]
...
def _reducer(sea: np.array, move: list[list[int, int]]):
    if move[0][0] == move[1][0]:
        x = move[0][0]
        for y in range(min(move[0][1], move[1][1]), max(move[0][1], move[1][1]) + 1):
            _add_move_in_sea(sea, x, y)
    elif move[0][1] == move[1][1]:
        y = move[0][1]
        for x in range(min(move[0][0], move[1][0]), max(move[0][0], move[1][0]) + 1):
            _add_move_in_sea(sea, x, y)
    else:
        for i in range(abs(move[0][0] - move[1][0]) + 1):
            x = move[0][0] + i if move[0][0] < move[1][0] else move[0][0] - i
            y = move[0][1] + i if move[0][1] < move[1][1] else move[0][1] - i
            _add_move_in_sea(sea, x, y)

    return sea


def _add_move_in_sea(sea: np.array, x: int, y: int) -> np.array:
    passages = sea.item((x, y)) + 1
    sea.itemset((x, y), passages)
